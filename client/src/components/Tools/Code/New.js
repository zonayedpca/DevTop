...
class New extends Component {
    ...
    updateStatus = () => {
        ...
        const { input } = codes;
        const enabled = Object.keys(input.data).every(
            oneData => input.data[oneData].code
        );
        ...
        return (
            <>
                <div className="code-new-desc">
                    <input
                        value={desc}
                        onChange={e => handleNewInput({ desc: e.target.value })}
                        name="desc"
                        ...
                        <div key={every} className="code-new-file">
                            <div className="code-new-file-name">
                                <input
                                    className="new-file"
                                    type="text"
                                    value={data[every] ? data[every].name : ''}
                                    onChange={e =>
                                        this.handleInput(e, every, 'name')
                                    }
                                    name={`file[${every}]`}
                                ...
                                {file.length > 1 && (
                                    <p
                                        className="code-file-remove"
                                        onClick={this.handleRemoveFile.bind(
                                            this,
                                            every
                                        )}
                                    >
                            ...
                            <div className="code-new-code-area">
                                <textarea
                                    className="new-code"
                                    value={data[every] ? data[every].code : ''}
                                    onChange={e =>
                                        this.handleInput(e, every, 'code')
                                    }
                                    name={`code[${every}]`}
                                ></textarea>
                            </div>
                ...
                <div className="code-new-action">
                    ...
                    <div>
                        <button
                            className={`code-btn-secret ${!enabled &&
                                `btn-disabled`}`}
                            onClick={this.handleSubmit.bind(this, 'secret')}
                        >
                            Secret
                        </button>
                        <button
                            className={`code-btn-public ${!enabled &&
                                `btn-disabled`}`}
                            onClick={this.handleSubmit.bind(this, 'public')}
                        >
                            Public
                        </button>
                    </div>
    ...
    render() {
        return <div className="code-new">{this.renderForm()}</div>;
    }
}

export default connect(
    ({ options, codes }) => ({ options, codes }),
    { getCode, createNewCode, handleNewInput }
)(New);
